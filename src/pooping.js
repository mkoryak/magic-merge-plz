...
const get = (url, data) =>
  new Promise((resolve, reject) =>
    needle.request('get', url, data, (err, response, body) =>
...
const extractExtract = data =>
  head(Object.values(data.query.pages))
    .extract;

const getRandomWikiExtract = () =>
  get(WIKIPEDIA_API_URL, RANDOM_EXTRACT_OPTS)
    .then(extractExtract);

////////////////////////////////////////////////////////////////////////////////

const extractTranslation = arr =>
  head(arr)
    .map(head)
    .join(' ');
...
const removeOffendingTokens = str =>
  str
    .replace(ELISION_PATTERN, ',')
    .replace(NEWLINE_PATTERN, '\\n');

const translator = (sl, tl) => q =>
  get(TRANSLATE_API_URL, Object.assign({ q, sl, tl }, TRANSLATE_BASE_OPTS))
    .then(removeOffendingTokens)
    .then(JSON.parse)
